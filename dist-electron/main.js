  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (isHttp2) {
      ...
      if (configTransport) {
        ...
        transport = isHttpsRequest ? https$2 : require$$3;
...
async function initDatabase() {
  const wasmPath = path$1.join(__dirname, "sql-wasm.wasm");
  let wasmBinary;
  try {
    wasmBinary = fs$1.readFileSync(wasmPath);
  } catch {
    const sqlJs = require("sql.js");
    wasmBinary = sqlJs.WASM;
  }
  const SQL = await initSqlJs({ wasmBinary });
  const dbPath = path$1.join(electron.app.getPath("userData"), "bili-copilot.db");
  let buffer;
  try {
    buffer = fs$1.readFileSync(dbPath);
  } catch {
    buffer = void 0;
  }
  ...
  db.saveToFile = () => {
    ...
    fs$1.writeFileSync(dbPath, Buffer.from(data));
  };
...
let loginKey = "";
electron.ipcMain.handle("login-get-qr", async () => {
  try {
...
electron.ipcMain.handle("login-check-status", async () => {
  var _a;
  ...
  try {
    ...
    if (code === 0 && ((_a = res.data.data) == null ? void 0 : _a.refresh_token)) {
...
electron.ipcMain.handle("logout", async () => {
  ...
  for (const c of cookies2) {
    await electron.session.defaultSession.cookies.remove("https://bilibili.com", c.name);
  }
  loginKey = "";
  return true;
});
electron.ipcMain.handle("is-logged-in", async () => {
  const cookies2 = await electron.session.defaultSession.cookies.get({ url: "https://bilibili.com", name: "SESSDATA" });
...
electron.ipcMain.handle("db-mark-up", async (_event, upData) => {
  try {
...
electron.ipcMain.handle("db-unmark-up", async (_event, mid) => {
  try {
...
electron.ipcMain.handle("db-get-marked-ups", async () => {
  try {
...
electron.ipcMain.handle("db-add-dynamics", async (_event, dynamics) => {
  try {
...
electron.ipcMain.handle("db-get-up-dynamics", async (_event, mid, limit = 50) => {
  try {
...
electron.ipcMain.handle("fetch-up-dynamics", async (_event, mid) => {
  try {
    const cookies2 = await electron.session.defaultSession.cookies.get({ url: "https://bilibili.com" });
    const cookieStr = cookies2.map((c) => `${c.name}=${c.value}`).join("; ");
    console.log("[fetch-up-dynamics] Cookie 长度:", cookieStr.length);
    const url2 = `https://api.bilibili.com/x/polymer/web-dynamic/v1/feed/space?host_mid=${mid}&platform=web&web_location=333.1387`;
    return new Promise((resolve) => {
      const req = https$2.get(url2, {
        headers: {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            if (parsed.code === 0) {
              const items = ((_a = parsed.data) == null ? void 0 : _a.items) || [];
              const dynamics = items.map((item) => {
                ...
                const desc = ((_a2 = dyn.desc) == null ? void 0 : _a2.text) || "";
                const major = dyn.major;
                let hasVideo = false, bvid = "", title = desc, pic = "", play = 0;
                if (major == null ? void 0 : major.archive) {
                  hasVideo = true;
                  bvid = major.archive.bvid || "";
                  title = major.archive.title || desc;
                  pic = major.archive.cover || "";
                  play = ((_c = (_b = dyn.stat) == null ? void 0 : _b.view) == null ? void 0 : _c.count) || 0;
                } else if (major == null ? void 0 : major.opus) {
                  const opus = major.opus;
                  title = opus.title || desc;
                  if (((_d = opus.pics) == null ? void 0 : _d.length) > 0) pic = opus.pics[0].url;
                  play = ((_f = (_e = dyn.stat) == null ? void 0 : _e.view) == null ? void 0 : _f.count) || 0;
                } else if (major == null ? void 0 : major.live_rcmd) {
                  return null;
                }
                const timestamp = ((_g = mod.module_author) == null ? void 0 : _g.pub_ts) || 0;
                return {
                  mid,
